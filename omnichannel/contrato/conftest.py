...
Como rodar (de omnichannel/):

    ../.venv/bin/python -m pytest contrato -q                           # Python (padrão)
    OMNI_CONTRATO_ALVO=php ../.venv/bin/python -m pytest contrato -q    # PHP (php -S + SQLite)

Variáveis:
    OMNI_CONTRATO_ALVO      python | php  (padrão python). O conftest sobe o
                            servidor numa porta livre, com banco SQLite novo,
                            sandbox ligado e a base do seed, e o derruba no fim.
    OMNI_CONTRATO_URL       usa um servidor JÁ no ar (não sobe nada). Ele
                            precisa ter a base do seed (admin/ana); os testes
                            que dependem da chave secreta são pulados.
    OMNI_CONTRATO_MYSQL     só no alvo php: "dsn|usuario|senha" de um banco
                            DESCARTÁVEL (o nome precisa conter "teste" ou
                            "contrato"); as tabelas são apagadas no início.
    OMNI_CONTRATO_ESTRITO   1 = rota ainda não implementada no alvo é FALHA,
                            não "skip" (use na validação final do porte).
    OMNI_CONTRATO_PHP       binário do PHP (padrão "php").

Provedores falsos (sem rede): os dois servidores leem OMNI_TESTE_PROVEDOR,
...
def _subir_python(pasta: Path, porta: int, ambiente: dict, log: Path) -> subprocess.Popen:
    ambiente.update(
        OMNI_BANCO_URL=f"sqlite:///{pasta / 'contrato.db'}",
        OMNI_CHAVE_SECRETA=CHAVE_SECRETA,
        OMNI_MODO_SANDBOX="1",
        OMNI_COLETOR_ATIVO="0",
        OMNI_PASTA_ANEXOS=str(pasta / "anexos"),
    )
...
def _config_php(pasta: Path) -> dict:
    ...
    mysql = os.environ.get("OMNI_CONTRATO_MYSQL")
    if mysql:
        ...
        if "teste" not in nome_banco and "contrato" not in nome_banco:
            raise RuntimeError("OMNI_CONTRATO_MYSQL precisa apontar um banco descartável (nome com 'teste' ou 'contrato')")
...
def _subir_php(pasta: Path, porta: int, ambiente: dict, log: Path) -> subprocess.Popen:
    ...
    ambiente.update(OMNI_CONFIG=str(arquivo), PHP_CLI_SERVER_WORKERS="4")
...
def servidor():
    ...
    externo = os.environ.get("OMNI_CONTRATO_URL")
    alvo = os.environ.get("OMNI_CONTRATO_ALVO", "python").lower()
    pasta = Path(tempfile.mkdtemp(prefix=f"omni-contrato-{alvo}-"))
    ...
    ambiente = {
        **os.environ,
        "OMNI_TESTE_PROVEDOR": str(provedor.arquivo),
        "OMNI_SENHA_ADMIN": ADMIN_SENHA,
    }
    ambiente.pop("OMNI_CONFIG", None)
    if alvo == "python":
        processo = _subir_python(pasta, porta, ambiente, log)
    elif alvo == "php":
        processo = _subir_php(pasta, porta, ambiente, log)
    else:
        raise pytest.UsageError(f"OMNI_CONTRATO_ALVO desconhecido: {alvo}")
