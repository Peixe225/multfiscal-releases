...
cp .env.example .env          # ajuste OMNI_CHAVE_SECRETA
...
qualquer API. Em produção, desligue com `OMNI_MODO_SANDBOX=false`, para que uma
...
A entrada tem dois caminhos: coleta IMAP a cada `OMNI_INTERVALO_COLETA`
...
```bash
cd omnichannel && python -m pytest
```
