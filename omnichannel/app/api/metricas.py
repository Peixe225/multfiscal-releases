...
def resumo(sessao: Sessao, _: AtendenteAtual) -> MetricasSaida:
    inicio = _inicio_do_dia()

    def contar(*condicoes) -> int:
        return sessao.scalar(select(func.count(Conversa.id)).where(*condicoes)) or 0
    ...
    mensagens_hoje = (
        sessao.scalar(select(func.count(Mensagem.id)).where(Mensagem.criada_em >= inicio)) or 0
    )
    ...
    tempos = sessao.execute(
        select(Conversa.criada_em, Conversa.primeira_resposta_em).where(
            Conversa.primeira_resposta_em.is_not(None)
        )
    ).all()
    media = None
    if tempos:
