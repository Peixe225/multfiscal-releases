"""Sobe o OmniChannel 2 com um comando só, sem precisar conhecer venv, pip ou uvicorn.
...
if sys.version_info < VERSAO_MINIMA:
    sys.stderr.write(
        "\nO OmniChannel 2 precisa do Python %d.%d ou mais novo; este é o %d.%d (%s).\n"
...
ARQUIVO_LOG = RAIZ / "omnichannel.log"
...
ARQUIVO_TRAVA = RAIZ / "omnichannel.lock"
...
DADOS_LOCAIS = (RAIZ / "omnichannel.db", RAIZ / "anexos")
...
# a que o scripts/seed.py usa quando falta OMNI_SENHA_ADMIN
SENHA_ADMIN_PADRAO = "admin123"
EMAIL_ADMIN = "admin@multfiscal.com.br"
LOGINS_EXEMPLO = [
    (EMAIL_ADMIN, None, "administrador"),  # senha vem de OMNI_SENHA_ADMIN
...
def pegar_trava() -> int | None:
    ...
    try:
        ...
        raise ErroLancador(
            ...
            "A pasta do OmniChannel precisa aceitar gravação: copie-a para a sua pasta de usuário."
        ) from None
    try:
        if WINDOWS:
            import msvcrt
...
def avisar_outra_copia(sem_navegador: bool) -> int:
    ...
    if isinstance(porta, int) and _saude_responde(porta):
        ...
        aviso(f"O OmniChannel já está rodando nesta pasta: {endereco}")
        aviso("Para pará-lo, use a janela em que ele foi aberto (Ctrl+C).")
        if not sem_navegador:
            abrir_navegador(endereco)
        return 0
    aviso()
    aviso("Outra janela já está abrindo o OmniChannel nesta pasta.")
    ...
    if PASTA_VENV.exists() and not _venv_utilizavel(python):
        ...
        try:
            ...
            raise ErroLancador(
                ...
                "Feche os outros programas que usam o OmniChannel e rode de novo."
            ) from None
...
def _com_chave_nova(linhas: list[str]) -> list[str]:
    """Troca toda definição de OMNI_CHAVE_SECRETA por uma só, com chave nova, no lugar da primeira."""
    nova = f"OMNI_CHAVE_SECRETA={secrets.token_urlsafe(48)}"
    resultado = []
    for linha in linhas:
        if _nome_na_linha(linha) != "OMNI_CHAVE_SECRETA":
...
def _com_origens_fechadas(linhas: list[str]) -> list[str]:
    ...
    fechada = [
        "# Vazio: só as páginas do próprio OmniChannel. Exemplo: [\"https://www.seusite.com.br\"]",
        "OMNI_ORIGENS_PERMITIDAS=[]",
    ]
    if any(_nome_na_linha(linha) == "OMNI_ORIGENS_PERMITIDAS" for linha in linhas):
        return linhas
    for indice, linha in enumerate(linhas):
        if linha.lstrip("# ").startswith("OMNI_ORIGENS_PERMITIDAS="):
...
def _gravar_env(linhas: list[str], exclusivo: bool) -> None:
    ...
    try:
        ...
        raise ErroLancador(
            ...
            "Troque a linha OMNI_CHAVE_SECRETA=... por OMNI_CHAVE_SECRETA=<um texto longo e aleatório> e rode de novo."
        ) from None
...
def preparar_env() -> None:
    ...
    if _chave_fraca(_interpretar_env(linhas).get("OMNI_CHAVE_SECRETA")):
...
def ambiente_dos_processos() -> dict[str, str]:
    ...
    # ambiente: sem isto, trocar OMNI_SENHA_ADMIN no .env não teria efeito.
    # Vazia conta como ausente: o seed gravaria um administrador sem senha.
    if not ambiente.get("OMNI_SENHA_ADMIN"):
        ambiente.pop("OMNI_SENHA_ADMIN", None)
        if valores.get("OMNI_SENHA_ADMIN"):
            ambiente["OMNI_SENHA_ADMIN"] = valores["OMNI_SENHA_ADMIN"]

    # variável do sistema vence o .env: vazia ou a de exemplo, ela anularia a
    # chave que o preparar_env acabou de garantir
    if "OMNI_CHAVE_SECRETA" in ambiente and _chave_fraca(ambiente["OMNI_CHAVE_SECRETA"]):
        del ambiente["OMNI_CHAVE_SECRETA"]
        aviso("Ignorei a OMNI_CHAVE_SECRETA do ambiente do sistema (vazia ou a de exemplo): vale a do .env.")
    ...
    if "OMNI_ORIGENS_PERMITIDAS" not in ambiente and "OMNI_ORIGENS_PERMITIDAS" not in valores:
        ambiente["OMNI_ORIGENS_PERMITIDAS"] = "[]"
...
def abrir_log() -> tuple[Path, object]:
    try:
        ...
        alternativo = Path(tempfile.gettempdir()) / f"omnichannel-{os.getpid()}.log"
...
def subir_servidor(python: Path, porta: int, ambiente: dict[str, str], log) -> subprocess.Popen:
    comando = [
        ...
        # válido por horas, em texto puro, no omnichannel.log
        "--no-access-log",
    ]
...
def logins_que_valem(tentar, senha_admin: str) -> list[tuple[str, str | None, str]]:
    ...
    O seed não mexe numa base que já existe, então o OMNI_SENHA_ADMIN do .env
...
def quadro(porta: int, chave_webchat: str, logins, senha_admin: str, log: Path) -> None:
    ...
    if senha_admin != SENHA_ADMIN_PADRAO and senha_valida != senha_admin:
        # quem trocou OMNI_SENHA_ADMIN depois da primeira carga acha que a
        # senha mudou; o painel ainda não tem tela de troca, só a API
        aviso("Atenção: o OMNI_SENHA_ADMIN do .env não é a senha do administrador desta")
...
def ler_argumentos(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Prepara e sobe o OmniChannel 2 (ambiente, .env, base de exemplo e servidor)."
    )
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        senha_admin = ambiente.get("OMNI_SENHA_ADMIN") or SENHA_ADMIN_PADRAO
