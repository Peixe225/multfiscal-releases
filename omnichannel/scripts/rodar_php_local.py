...
recebe o caminho do config de desenvolvimento por OMNI_CONFIG.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sobe o OmniChannel 2 em PHP com SQLite")
    ...
    banco = pasta / "omnichannel.sqlite"
    config = _config(pasta, banco)

    ambiente = {**os.environ, "OMNI_CONFIG": str(config), "PHP_CLI_SERVER_WORKERS": "4"}
    ...
    try:
        for _ in range(50):
            try:
                ...
                time.sleep(0.2)
        print()
        print(f"  OmniChannel 2 (PHP) no ar: {base}/painel")
