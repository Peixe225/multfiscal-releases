...
namespace OmniChannel\Simulador;
...
use OmniChannel\Canais\Adaptador;
use OmniChannel\Canais\CanalNaoSuportado;
use OmniChannel\Canais\Canais;
use OmniChannel\Canais\Registro;
...
final class Rotas
{
    ...
    private static function payloadEmail(string $endereco, ?string $nome, string $conteudo, ?string $assunto): array
    {
        ...
        $payload = [
            'from' => $remetente,
            'text' => $conteudo,
            'message-id' => '<' . bin2hex(random_bytes(16)) . '@simulador.omnichannel>',
        ];
