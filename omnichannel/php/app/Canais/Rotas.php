...
final class Rotas
{
    ...
    public static function verCredenciais(Requisicao $req, array $p): array
    {
        ...
        return [
            ...
            // webhook de entrada (X-Omni-Token ou ?token=). O de um WhatsApp
    ...
    public static function receberWebhook(Requisicao $req, array $p): array
    {
        ...
        if (!isset($cabecalhos['x-omni-token']) && $token !== null && $token !== '') {
            $cabecalhos['x-omni-token'] = $token;
        }
     ...
     */
    public static function gravarRecebida(array $canal, \OmniChannel\Atendimento\MensagemRecebida $recebida): ?array
    {
        try {
