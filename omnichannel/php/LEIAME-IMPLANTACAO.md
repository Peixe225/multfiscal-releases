...
- **Arquivos**: `omnichannel2/config.php` (guarde num cofre de senhas ou numa
...
| Perdi o `config.php` | Copie `omnichannel2/config.exemplo.php` para `omnichannel2/config.php` e troque os dados do banco (`dsn`, `usuario`, `senha`), a `chave_secreta` (64 caracteres aleatórios: `php -r "echo bin2hex(random_bytes(32));"`; uma chave nova só desloga todo mundo) e a `url_publica`. Mantenha a linha `'pasta_web' => __DIR__ . '/web'`: sem ela, painel e widget respondem 404. |
...
2. Crie `omnichannel/.env` com:
   - `OMNI_MODO_SANDBOX=0` e `OMNI_DEMO=0`;
   - `OMNI_SENHA_ADMIN=<senha forte>`;
   - `OMNI_ORIGENS_PERMITIDAS=["https://oprojeto.online","https://www.oprojeto.online"]`
     ...
     `OMNI_ORIGENS_PERMITIDAS=[]`, e o navegador bloqueia o balão em
     oprojeto.online (CORS) — ele aparece, mas não abre conversa;
   - `OMNI_CHAVE_SECRETA`: **só** se for levar o banco da Hostinger junto,
...
3. `OMNI_DEMO=0 docker compose up -d --build` (sobe em 127.0.0.1:8000; com
   `OMNI_DEMO=0` o seed roda com `--producao` e não cria a atendente de exemplo
   ...
   administrador é `admin@multfiscal.com.br` com a `OMNI_SENHA_ADMIN`. Numa base
   ...
   - entre como `admin@multfiscal.com.br` com a `OMNI_SENHA_ADMIN`;
