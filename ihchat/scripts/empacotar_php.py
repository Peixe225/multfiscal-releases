"""Monta o pacote de deploy do backend PHP para a Hostinger.
    ...
    ../.venv/bin/python scripts/empacotar_php.py            # dist/ihchat/ + dist/ihchat-<versão>.zip
    ../.venv/bin/python scripts/empacotar_php.py --sem-zip
    ../.venv/bin/python scripts/empacotar_php.py --saida /outra/pasta

O pacote é a pasta que vai inteira para public_html/ihchat/:

    ihchat/.htaccess        se o subdomínio apontar para cá, reescreve para public/
    ihchat/app/             código PHP (+ .htaccess que nega tudo)
    ihchat/public/          DocumentRoot: index.php, instalar.php, .htaccess e .user.ini
    ...
    ihchat/web/             o front (cópia de app/web, a fonte única), FORA do public:
    ...
    ihchat/cron.php, console.php
    ihchat/config.exemplo.php  o modelo, com 'pasta_web' => __DIR__ . '/web' ATIVO: um
                                  config.php refeito à mão a partir dele acha o front (sem
                                  essa linha, /painel e /widget.js dariam 404)
    ihchat/VERSAO           versão, commit e data do pacote
...
NEGAR_TUDO = """# Código do IHchat: nunca servido pela web.
...
def montar(saida: Path) -> tuple[Path, dict]:
    """Monta saida/ihchat do zero e devolve (pasta, manifesto)."""
...
def _ajustar_modelo_de_config(modelo: Path) -> None:
    ...
    no pacote o front mora em ihchat/web. Só o instalador grava a linha:
    ...
    linhas[fim:fim] = [
        "    // front (painel, widget): no pacote fica em ihchat/web, FORA do public/",
...
def zipar(pacote: Path, destino: Path) -> Path:
    """Zip com ihchat/ na raiz, em ordem estável.
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Empacota o IHchat (PHP) para a Hostinger")
    parser.add_argument("--saida", type=Path, default=RAIZ / "dist", help="pasta de saída (padrão: ihchat/dist)")
