"""Envia o pacote do IHchat para a Hostinger (public_html/ihchat/).
    ...
    #    pelo subdomínio E pelo domínio principal (public_html/ihchat fica
    ...
    ../.venv/bin/python scripts/implantar_hostinger.py --conferir https://atendimento.oprojeto.online \
        --conferir-pasta https://oprojeto.online/ihchat     # (é o padrão deduzido)
...
O {caminho} é relativo a public_html (ex.: ihchat/app/autoload.php).
...
ihchat/dados/instalacao.codigo (fora do public) e mostrado UMA vez.
"""
...
def normalizar_caminho(caminho: str, destino: str) -> str:
    """'public_html/ihchat/app/X.php', 'ihchat/app/X.php' ou '/app/X.php' -> 'app/X.php'."""
...
def _buscar(
    ...
    pedido = urllib.request.Request(
        url, method=metodo, headers={"User-Agent": "ihchat-conferencia", **(cabecalhos or {})}
    )
    try:
...
# Dentro de public_html/ihchat/: nada disto pode sair pela web. Pelo
...
INTERNOS = (
    ...
    "dados/", "dados/instalacao.codigo", "dados/instalacao.tentativas", "dados/ihchat.sqlite",
    "dados/logs/", "dados/anexos/", "web/painel.html", "public/web/painel.html",
)


def dominio_da_pasta(base: str, destino: str = "ihchat") -> str | None:
    """https://atendimento.oprojeto.online -> https://oprojeto.online/ihchat (o domínio pai)."""
...
def conferir_pasta(pasta: str, deduzida: bool = False) -> list[Resultado]:
    ...
    for caminho in INTERNOS:
        ...
        if status == 200:
            ...
            resultados.append(("FALHA", f"{url} ACESSÍVEL (200{pista}): o .htaccess de ihchat/ não protegeu"))
        else:
...
def conferir_widget_em_outro_site(base: str, origem: str) -> list[Resultado]:
    ...
    Com a origem bloqueada (ex.: IHCHAT_ORIGENS_PERMITIDAS=[] na VPS), o balão
    ...
    resultados.append((
        ...
        + ("" if liberada else "; libere a origem: origens_permitidas no config.php, IHCHAT_ORIGENS_PERMITIDAS na VPS"),
    ))
...
def conferir_no_ar(base: str, origem: str | None = None) -> list[Resultado]:
    ...
    for caminho in ("/config.php", "/app/autoload.php", "/app/Nucleo/Config.php", "/cron.php", "/console.php",
                    "/dados/instalacao.codigo", "/dados/ihchat.sqlite", "/.htaccess", "/public/../config.php",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Envia dist/ihchat para public_html/ihchat via TUS")
    parser.add_argument("--pasta", type=Path, default=DIST / "ihchat", help="pacote gerado pelo empacotar_php.py")
    parser.add_argument("--destino", default="ihchat", help="pasta dentro de public_html (padrão: ihchat)")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="a pasta pelo domínio principal (ex.: https://oprojeto.online/ihchat); "
