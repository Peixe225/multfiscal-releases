...
1. scripts/empacotar_php.py monta dist/ihchat (sem segredo, sem teste);
...
@pytest.fixture
def pasta():
    caminho = Path(tempfile.mkdtemp(prefix="ihchat-pacote-"))
    yield caminho
...
def test_pacote_sem_segredo_sem_teste_e_com_o_front(pasta):
    ...
    # zip: mesma lista, com ihchat/ na raiz, e a data real de cada arquivo
    zips = list((pasta / "dist").glob("ihchat-*.zip"))
    assert len(zips) == 1
    with zipfile.ZipFile(zips[0]) as z:
        assert {n.removeprefix("ihchat/") for n in z.namelist()} == caminhos
        for info in z.infolist():
            real = time.localtime((pacote / info.filename.removeprefix("ihchat/")).stat().st_mtime)
...
def test_zip_extraido_por_cima_entrega_o_js_novo(pasta):
    ...
    extrair_como_o_unzip(next((pasta / "dist").glob("ihchat-*.zip")), public_html)
    recebido = public_html / "ihchat"
...
def test_arquivo_fora_do_pacote_avisado_em_todo_envio_ate_confirmar(pasta, tus):
    ...
    for extra in ((), (), ("--tudo",), ("--simular",)):
        ...
        assert "public_html/ihchat/app/Util.php" in resultado.stdout
    assert (tus.raiz / "ihchat" / "app" / "Api" / "Antiga.php").exists()  # a API de upload não apaga

    # o dono apagou a rota pelo Gerenciador de Arquivos e confirma (qualquer forma do caminho)
    confirmado = implantar(tus, pacote, "--ja-apaguei", "public_html/ihchat/app/Api/Antiga.php")
...
def test_dois_sites_no_mesmo_servidor_de_arquivos_nao_dividem_o_registro(pasta, tus):
    ...
    assert (tus.raiz / "siteB" / "ihchat" / "app" / "X.php").exists()
...
def test_credencial_errada_para_na_hora(pasta, tus):
    ...
    resultado = implantar(tus, pasta / "dist" / "ihchat", auth="chave-errada")
...
def test_sem_variaveis_de_ambiente_nao_envia(pasta):
    ...
    ambiente = {k: v for k, v in os.environ.items() if not k.startswith("HOSTINGER_")}
    resultado = subprocess.run(
        [PYTHON, "scripts/implantar_hostinger.py", "--pasta", str(pasta / "dist" / "ihchat")],
    ...
    simulado = subprocess.run(
        [PYTHON, "scripts/implantar_hostinger.py", "--pasta", str(pasta / "dist" / "ihchat"), "--simular"],
        cwd=RAIZ, env=ambiente, capture_output=True, text=True, timeout=60,
    )
...
def test_implantar_instalar_e_atualizar_ponta_a_ponta(pasta, tus):
    ...
    assert posts[0].startswith("api/upload/ihchat/app/Banco/")
    ...
    try:
        with servidor.cliente() as c:
            ...
            assert [c for m, c in tus.log if m == "POST"] == [
                "api/upload/ihchat/app/Banco/Migracoes/M29991231_2359_TesteDeploy.php",
                "api/upload/ihchat/app/Atendimento/Mensagens.php",
            ]
            banco = recebido / "dados" / "ihchat.sqlite"
            ...
            # widget bloqueado por CORS (o caso da VPS com IHCHAT_ORIGENS_PERMITIDAS=[]): a conferência acusa
