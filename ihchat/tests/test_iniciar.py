...
def pasta(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(iniciar, "ARQUIVO_TRAVA", raiz / "ihchat.lock")
    monkeypatch.setattr(iniciar, "ARQUIVO_LOG", raiz / "ihchat.log")
    monkeypatch.setattr(iniciar, "PASTA_VENV", raiz / ".venv")
    monkeypatch.setattr(iniciar, "DADOS_LOCAIS", (raiz / "ihchat.db", raiz / "anexos"))
    for nome in ("IHCHAT_SENHA_ADMIN", "IHCHAT_CHAVE_SECRETA", "IHCHAT_ORIGENS_PERMITIDAS"):
...
def test_quadro_nao_mostra_a_senha_do_env_trocada_depois_da_primeira_carga(cliente, monkeypatch, capsys):
    ...
    monkeypatch.setenv("IHCHAT_SENHA_ADMIN", "admin123")
    semear()
    # o dono troca IHCHAT_SENHA_ADMIN no .env e roda de novo: o seed não mexe
    # numa base que já existe, então quem vale continua sendo a admin123
    monkeypatch.setenv("IHCHAT_SENHA_ADMIN", "segredo123")
    semear()
    capsys.readouterr()
    ...
    iniciar.quadro(8000, "", logins, "segredo123", Path("ihchat.log"))
...
def test_quadro_nao_mostra_senha_de_exemplo_que_ja_foi_trocada(cliente, monkeypatch, capsys):
    ...
    iniciar.quadro(8000, "", logins, "admin123", Path("ihchat.log"))
    saida = capsys.readouterr().out
...
def test_senha_do_admin_vazia_conta_como_ausente(pasta, monkeypatch):
    ...
    (pasta / ".env").write_text("IHCHAT_SENHA_ADMIN=a-do-env-123\n", encoding="utf-8")
    monkeypatch.setenv("IHCHAT_SENHA_ADMIN", "")
    assert iniciar.ambiente_dos_processos()["IHCHAT_SENHA_ADMIN"] == "a-do-env-123"

    (pasta / ".env").write_text("IHCHAT_SENHA_ADMIN=\n", encoding="utf-8")
    assert "IHCHAT_SENHA_ADMIN" not in iniciar.ambiente_dos_processos()
...
def test_segunda_copia_nao_mexe_no_venv_e_aponta_para_a_primeira(pasta, monkeypatch, capsys):
    ...
    try:
        ...
        assert "Outra janela já está abrindo o IHchat" in capsys.readouterr().out
...
def test_env_criado_por_outro_processo_no_meio_do_caminho_nao_vira_traceback(pasta, monkeypatch):
    ...
    do_outro = "IHCHAT_CHAVE_SECRETA=chave-que-o-outro-processo-acabou-de-gravar\n"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "IHCHAT_SENHA_ADMIN=admin123\n",
        "IHCHAT_CHAVE_SECRETA=\nIHCHAT_SENHA_ADMIN=admin123\n",
        'IHCHAT_CHAVE_SECRETA="troque-esta-chave-em-producao"\nIHCHAT_SENHA_ADMIN=admin123\n',
        # a última definição vence, como no python-dotenv que o app usa
        "IHCHAT_CHAVE_SECRETA=uma-chave-forte-mas-sobrescrita-123\n"
        "IHCHAT_SENHA_ADMIN=admin123\n"
        "export IHCHAT_CHAVE_SECRETA=troque-esta-chave-em-producao  # de exemplo\n",
    ],
)
def test_env_existente_com_chave_publica_ganha_chave_propria(pasta, capsys, conteudo):
    ...
    chave = valores["IHCHAT_CHAVE_SECRETA"]
    assert chave != iniciar.CHAVE_DE_EXEMPLO and len(chave) >= 40
    # o resto do arquivo fica como o dono deixou
    assert valores["IHCHAT_SENHA_ADMIN"] == "admin123"
    assert (pasta / ".env").read_text(encoding="utf-8").count("IHCHAT_CHAVE_SECRETA=") == 1
    ...
    iniciar.preparar_env()
    assert _valores_do_env(pasta)["IHCHAT_CHAVE_SECRETA"] == chave


def test_env_com_chave_propria_fica_intocado(pasta, capsys):
    conteudo = "# meu .env\nIHCHAT_CHAVE_SECRETA=minha-chave-longa-e-aleatoria-de-verdade\n"
    (pasta / ".env").write_text(conteudo, encoding="utf-8")
    iniciar.preparar_env()
...
def test_chave_publica_no_ambiente_do_sistema_nao_chega_ao_app(pasta, monkeypatch, valor):
    ...
    iniciar.preparar_env()
    monkeypatch.setenv("IHCHAT_CHAVE_SECRETA", valor)
    assert "IHCHAT_CHAVE_SECRETA" not in iniciar.ambiente_dos_processos()

    monkeypatch.setenv("IHCHAT_CHAVE_SECRETA", "uma-chave-forte-posta-no-sistema")
    assert iniciar.ambiente_dos_processos()["IHCHAT_CHAVE_SECRETA"] == "uma-chave-forte-posta-no-sistema"
...
def test_env_novo_nasce_com_cors_fechado(pasta):
    ...
    assert iniciar._interpretar_env(linhas)["IHCHAT_ORIGENS_PERMITIDAS"] == "[]"
    # uma definição só: a linha comentada do exemplo virou a de verdade
    assert [iniciar._nome_na_linha(linha) for linha in linhas].count("IHCHAT_ORIGENS_PERMITIDAS") == 1
    assert not any(linha.startswith("# IHCHAT_ORIGENS_PERMITIDAS=") for linha in linhas)


def test_cors_fica_fechado_sem_atropelar_quem_configurou_o_widget(pasta, monkeypatch):
    chave = "IHCHAT_CHAVE_SECRETA=chave-forte-de-um-env-de-antes-da-correcao\n"
    (pasta / ".env").write_text(chave, encoding="utf-8")
    assert iniciar.ambiente_dos_processos()["IHCHAT_ORIGENS_PERMITIDAS"] == "[]"

    # no app a variável de ambiente venceria o .env: quem já liberou o site do
    # widget ali não pode perder a liberação
    (pasta / ".env").write_text(chave + 'IHCHAT_ORIGENS_PERMITIDAS=["https://www.loja.com.br"]\n', encoding="utf-8")
    assert "IHCHAT_ORIGENS_PERMITIDAS" not in iniciar.ambiente_dos_processos()

    monkeypatch.setenv("IHCHAT_ORIGENS_PERMITIDAS", '["https://outra.com.br"]')
    assert iniciar.ambiente_dos_processos()["IHCHAT_ORIGENS_PERMITIDAS"] == '["https://outra.com.br"]'
...
def test_cors_fechado_recusa_pagina_de_outra_origem_e_mantem_o_painel(pasta, monkeypatch):
    ...
    monkeypatch.setenv("IHCHAT_ORIGENS_PERMITIDAS", iniciar.ambiente_dos_processos()["IHCHAT_ORIGENS_PERMITIDAS"])
    config = Configuracao()
...
def test_log_base_e_anexos_de_um_lancador_antigo_ficam_so_do_dono(pasta):
    for arquivo in (pasta / "ihchat.log", pasta / "ihchat.db"):
        arquivo.write_bytes(b"x")
        arquivo.chmod(0o644)
    (pasta / "anexos").mkdir(mode=0o755)

    iniciar.proteger_dados_antigos()
    assert _modo(pasta / "ihchat.db") == 0o600
    assert _modo(pasta / "anexos") == 0o700

    (pasta / "ihchat.log").chmod(0o644)
    caminho, log = iniciar.abrir_log()
    log.close()
    assert caminho == pasta / "ihchat.log"
...
def test_container_nunca_assina_com_a_chave_de_exemplo(tmp_path, recebida, esperada):
    ...
    _executavel(bin_ / "uvicorn", 'echo "assinando com: $IHCHAT_CHAVE_SECRETA"\n')
    ambiente = {"PATH": f"{bin_}{os.pathsep}/usr/bin{os.pathsep}/bin", "IHCHAT_DEMO": "0"}
    if recebida is not None:
        ambiente["IHCHAT_CHAVE_SECRETA"] = recebida
...
def test_container_nao_cria_admin_sem_senha(tmp_path, recebida, esperada):
    # o env_file do compose passa um IHCHAT_SENHA_ADMIN= vazio adiante, e o seed
    # só usa a admin123 quando a variável não existe
    dados = tmp_path / "dados"
    dados.mkdir()
    bin_ = tmp_path / "bin"
    bin_.mkdir()
    _executavel(bin_ / "python", 'if [ "$1" = -m ]; then echo "senha do seed: ${IHCHAT_SENHA_ADMIN-ausente}"; fi\n')
    _executavel(bin_ / "uvicorn", "true\n")
    ambiente = {"PATH": f"{bin_}{os.pathsep}/usr/bin{os.pathsep}/bin", "IHCHAT_CHAVE_SECRETA": "uma-chave-forte"}
    if recebida is not None:
        ambiente["IHCHAT_SENHA_ADMIN"] = recebida
...
def test_imagem_nasce_com_cors_fechado():
    ...
    assert re.search(r"^\s+IHCHAT_ORIGENS_PERMITIDAS=\[\] \\$", texto, re.MULTILINE)
...
def test_dockerignore_deixa_dados_e_segredos_fora_em_qualquer_pasta():
    ...
    # seed rodado de dentro de scripts/ grava a base em scripts/ihchat.db
    for segredo in (
        ".env", "ihchat.db", "ihchat.log", "anexos/1/doc.pdf", "ihchat.lock",
        "scripts/ihchat.db", "scripts/ihchat.db-journal", "scripts/.env",
        "scripts/ihchat.log", "app/anexos/doc.pdf", "app/__pycache__/main.cpython-311.pyc",
    ):
...
def test_seed_de_producao_nao_cria_a_atendente_de_senha_publica(cliente, monkeypatch, capsys):
    """Na VPS (IHCHAT_DEMO=0 no container) a base nasce só com o administrador,
    como a do instalador PHP: ana12345 está no repositório."""
    from scripts.seed import semear

    monkeypatch.setenv("IHCHAT_SENHA_ADMIN", "senha-do-dono-9")
