...
@pytest.mark.parametrize(
    "descricao, trecho",
    [(CONFLITO_WEBHOOK, "webhook ativo"), (CONFLITO_INSTANCIA, "outra cópia do IHchat")],
)
def test_conflito_409_e_registrado_sem_derrubar_outro_canal(caplog, descricao, trecho):
    ...
    transporte(responder)
    with caplog.at_level(logging.WARNING, logger="ihchat.coletor"):
        assert coletar_uma_vez() == 1

    avisos = [r.getMessage() for r in caplog.records if r.name == "ihchat.coletor"]
    assert len(avisos) == 1
...
def test_mensagem_com_defeito_e_pulada_sem_levar_as_do_lote(monkeypatch, caplog):
    ...
    with caplog.at_level(logging.ERROR, logger="ihchat.coletor"):
        assert coletar_uma_vez() == 2
    coletar_uma_vez()
...
def test_falha_intermitente_fica_visivel_e_nao_alterna_no_log(caplog):
    ...
    with caplog.at_level(logging.INFO, logger="ihchat.coletor"):
        for _ in range(6):
            coletar_uma_vez()

    situacao = coletor.situacao(canal.id)
    # o "Testar conexão" diria "Conectado": é isto que mostra que não está recebendo
    assert situacao["recebendo"] is False and "outra cópia do IHchat" in situacao["erro"]
    assert situacao["erro_desde"] is not None and situacao["ultima_coleta_ok"] is not None
    linhas = [r.getMessage() for r in caplog.records if r.name == "ihchat.coletor"]
...
def test_laco_continua_quando_um_canal_falha_e_nao_repete_o_aviso(caplog):
    ...
    assert len(avisos) == 1 and "outra cópia do IHchat" in avisos[0]
