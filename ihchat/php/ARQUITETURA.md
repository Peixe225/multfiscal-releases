# IHchat em PHP: arquitetura e convenções

O backend PHP implementa **o mesmo contrato HTTP** do app Python
(`ihchat/app`): mesmas rotas, métodos, JSON, códigos de status e mensagens.
...
php/app/                código (namespace IHchat\..., autoload PSR-4 próprio)
...
namespace IHchat\Api;

use IHchat\Auth\Auth;
use IHchat\Banco\Banco;
use IHchat\Eventos\Eventos;
use IHchat\Nucleo\{ErroHttp, Requisicao, Resposta, Roteador, Validador};
...
**Provedor falso da suíte de contrato**: com `IHCHAT_TESTE_PROVEDOR=<arquivo.json>`
...
- Contrato (HTTP, os dois servidores): em `ihchat/`,
  `../.venv/bin/python -m pytest contrato -q` (Python) e
  `IHCHAT_CONTRATO_ALVO=php ../.venv/bin/python -m pytest contrato -q` (PHP).
