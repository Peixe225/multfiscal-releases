...
final class Instalador
{
    ...
    private const SENHAS_OBVIAS = ['1234567890', 'senha12345', 'password123', 'admin12345', 'qwertyuiop', 'ihchat'];
    ...
    private static function instalar(Requisicao $req, bool $json): Resposta
    {
        ...
        if (!is_dir($pasta) && !@mkdir($pasta, 0775, true)) {
            error_log("IHchat instalação: não consegui criar {$pasta}");
    ...
    private static function valoresDoConfig(array $pedido, string $pasta): array
    {
        ...
        // no pacote de deploy o front fica em ihchat/web, FORA do public
        ...
        if ($pedido['banco_tipo'] === 'mysql') {
            ...
            $arquivo = ['driver' => 'sqlite', 'dsn' => 'dados/ihchat.sqlite'] + $comum + ['pasta_dados' => 'dados'];
            $memoria = ['driver' => 'sqlite', 'dsn' => 'sqlite:' . $pasta . '/ihchat.sqlite'] + $comum + ['pasta_dados' => $pasta];
        }
    ...
    /**
     * A pasta pela qual o instalador foi aberto (ex.: "/ihchat" em
     * https://oprojeto.online/ihchat/instalar), ou null se foi pela raiz.
     */
    ...
    private static function registrar(string $mensagem, ?\Throwable $erro = null, #[\SensitiveParameter] string $segredo = ''): void
    {
        ...
        if (is_dir($pasta) || @mkdir($pasta, 0775, true)) {
            @file_put_contents($pasta . '/ihchat-' . gmdate('Y-m-d') . '.log', $linha, FILE_APPEND | LOCK_EX);
        }
        error_log('IHchat ' . rtrim($linha));
    }
