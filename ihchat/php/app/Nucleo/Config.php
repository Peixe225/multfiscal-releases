 ...
 * IHCHAT_CONFIG aponta outro arquivo: é assim que a suíte de contrato e o
...
final class Config
{
    ...
    private function __construct(array $valores, string $arquivo)
    {
        ...
        $this->nome_aplicacao = (string) ($valores['nome_aplicacao'] ?? 'IHchat');
        $this->versao = (string) ($valores['versao'] ?? '2.0.0');

        $this->driver = strtolower((string) ($valores['driver'] ?? 'sqlite'));
        $this->dsn = (string) ($valores['dsn'] ?? ('sqlite:' . $raizPhp . '/dados/ihchat.sqlite'));
    ...
    {
        $doAmbiente = getenv('IHCHAT_CONFIG');
