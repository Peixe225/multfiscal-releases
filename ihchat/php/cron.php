 ...
 *
 *   /usr/bin/php /home/<usuario>/domains/<dominio>/public_html/ihchat/cron.php
 *
 * (o caminho exato aparece no Gerenciador de Arquivos; o cron.php fica na raiz
 * de ihchat/, FORA do public/, e recusa ser chamado pela web).
 *
 ...
 */
function ihchat_bytes_do_ini(string $valor): int
{
    $valor = trim($valor);
    if ($valor === '' || $valor === '-1') {
        return -1;
...
$limiteAtual = ihchat_bytes_do_ini((string) ini_get('memory_limit'));
...
// app/Tarefas/X.php -> IHchat\Tarefas\X; app/Canais/Tarefas/X.php -> IHchat\Canais\Tarefas\X
$tarefas = [];
foreach (array_merge(glob(__DIR__ . '/app/Tarefas/*.php') ?: [], glob(__DIR__ . '/app/*/Tarefas/*.php') ?: []) as $arquivo) {
    ...
    $tarefas[basename($arquivo, '.php')] = 'IHchat\\' . str_replace('/', '\\', $relativo) . '\\' . basename($arquivo, '.php');
}
ksort($tarefas);
