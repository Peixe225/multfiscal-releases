...
const CHAVE_PREFERENCIAS = "ihchat_simulador";

const estado = {
  token: lerArmazenado("ihchat_token"),
  canais: [],
  personaId: "ian",
  canalPorPersona: {},
  ...
  eventos: null, // IHchatEventos: o cursor garante que nada se perde ao reconectar
  naoVistas: 0,
};
...
$("#form-login").addEventListener("submit", async (evento) => {
  ...
  try {
    ...
    gravarArmazenado("ihchat_token", token);
    estado.token = token;
    await iniciar();
  } catch (erro) {
...
$("#trocar").addEventListener("click", () => {
  gravarArmazenado("ihchat_token", null);
  estado.token = null;
  mostrarLogin();
});
